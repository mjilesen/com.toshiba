...
function convertBytesToState(bytes) {
  const new_state = bytesToHex(bytes);
...
function bytesToHex(byteArray) {
  const value = byteArray.reduce((output, elem) => (output + (`0${elem.toString(16)}`).slice(-2)),
    '');
...
function setTargetTemperatureToRaw(device) {
  const target_temperature = device.getCapabilityValue(Constants.CapabilityTargetTemperatureInside);
