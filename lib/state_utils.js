...
function setModeToRaw( device){
  const target_ac_mode = device.getCapabilityValue( Constants.CapabilityTargetACMode )
  let raw = Constants.NONE_VAL;
...
function setFanModeToRaw( device){
  ...
  switch( target_fan_mode ){
    ...
    break;
    case Constants.FanMode_None: raw = 0x31;
    break;
    case Constants.FanMode_Quiet: raw = 0x32;
    break;
...
function setSwingMode( device, raw ){
  if ( raw != Constants.NONE_VAL ){
    ...
    device.setCapabilityValue( Constants.CapabilityTargetSwingMode, status).catch(this.error);
  }
}

function setSwingModeToRaw( device){
  const target_swing_mode = device.getCapabilityValue( Constants.CapabilityTargetSwingMode )
  let raw = Constants.NONE_VAL;
...
function setPowerSelectionToRaw( device){
  ...
  switch( target_power_mode ){
    ...
    break;
    case Constants.PowerSelection_power_75: raw = 0x48;
    break;
