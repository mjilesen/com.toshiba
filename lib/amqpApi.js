...
function printResultFor(op, deviceUniqueID, message ) {
  return function printResult(err, res) {
    if (err) this.Driver.homey.app.logInformation(`AMQP sendMessage`, `Send``${op} error: ${err.toString()}`);
    else{
      this.Driver.homey.app.logInformation( `AMQP sendMessage`, `Send: ${deviceUniqueID} ${message}`);
    }
  };
}
...
module.exports = class AmqpApi extends SimpleClass {
  ...
  async sendMessage(message, deviceUniqueID) {
    ...
    this.client.sendEvent(msg, printResultFor('send', deviceUniqueID, message ));
    this.Driver.homey.app.logStates(`Send: ${deviceUniqueID} ${message}`);
  }
