...
module.exports = class AmqpApi extends SimpleClass {
        ...
        if (device) {
          switch (command) {
            case Constants.CMD_HEARTBEAT:
              ...
              this.Driver.homey.app.logStates(`Received: ${command} ${payload.payload.iTemp} ${payload.payload.oTemp}`);
              break;
            case Constants.CMD_FCU_FROM_AC:
              device.updateState(payload.payload.data);
              this.Driver.homey.app.logStates(`Received: ${command} ${payload.payload.data}`);
              break;
  ...
  async sendMessage(message, DeviceUniqueID) {
    ...
    this.client.sendEvent(msg, printResultFor('send'));
    this.Driver.homey.app.logStates(`Send: ${message}`);
  }
