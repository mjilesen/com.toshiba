...
exports.getStatusResult = (device, isAutocomplete) => {
  const capability = device.getStoreValue(Constants.StoredCapabilityTargetACMode);
  const statuss = Features.acModes.find(element => element.capability === capability);
  statuss.modes.push(Constants.StatusOff);
  statuss.modes.push(Constants.StatusCleaning);

  if (isAutocomplete) {
    return transformToAutocomplete(statuss.modes);
  }
  return statuss;
};
