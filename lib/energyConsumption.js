...
module.exports = class EnergyConsumption extends SimpleClass {
  ...
  async getNextEnergyConsumption(device, date) {
    ...
    const previousHour = await device.getStoreValue( Constants.StoredEnergyPreviousHourEnergy );
    const previousHourEnergy = await device.getStoreValue( Constants.StoredEnergyPreviousHour );
    ...
    if ( previousHour && hour != previousHour ){
      if( hour === 0){
        ...
        addedEnergy = this.getEnergy( energyPreviousDay, 23 ) - ( previousHourEnergy?previousHourEnergy:0 ) + newEnergy ;
      }
      else{
        // add remaining from previous hour
        addedEnergy = this.getEnergy( energy, hour-1 ) - ( previousHourEnergy?previousHourEnergy:0 ) + newEnergy ;
      }   
    }
    else{
