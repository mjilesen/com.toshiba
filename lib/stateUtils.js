...
const Selections = require('./flowSelections.js');
...
async function setOutsideTemperatureFromRaw(device, value) {
  if (value < 255) {
    await device.setCapabilityValue(Constants.CapabilityMeasureTemperaturOutside, value).catch(device.error);
  }
}

async function setInsideTemperatureFromRaw(device, value) {
  if (value < 255) {
    await device.setCapabilityValue(Constants.CapabilityMeasureTemperatureInside, value).catch(device.error);
  }
}
