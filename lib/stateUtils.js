...
async function setOnOfFromRaw(device, raw) {
  ...
  await device.setCapabilityValue(Constants.CapabilityOnOff, status).catch(device.error);
}

async function setTargetTemperatureFromRaw(device, raw, is8C) {
  if (raw !== Constants.NONE_VAL) {
    // if 8c mode then set the 8c target temperature, otherwise the defaults
    if (is8C) {
      await device.setCapabilityValue(Constants.CapabilityTargetTemperatureInside_8c, raw).catch(device.error);
    } else {
      await device.setCapabilityValue(Constants.CapabilityTargetTemperatureInside, raw).catch(device.error);
    }
  }
}

function setTargetTemperatureToRaw(device) {
  let targetTemperature = device.getCapabilityValue(Constants.CapabilityTargetTemperatureInside);
  if (device.hasCapability(Constants.CapabilityTargetTemperatureInside_8c) && device.getCapabilityValue(Constants.CapabilityTargetMeritA) === Constants.MeritA_Heating_8C) {
    targetTemperature = device.getCapabilityValue(Constants.CapabilityTargetTemperatureInside_8c);
  }
...
async function setModeFromRaw(device, raw) {
  if (raw !== Constants.NONE_VAL) {
    ...
    if (allowedValues.modes.find(element => element === status)) {
      ...
      await device.setCapabilityValue(capability, status).catch(device.error);
    }
  }
}
...
async function setFanModeFromRaw(device, raw) {
  if (raw !== Constants.NONE_VAL) {
    ...
    await device.setCapabilityValue(Constants.CapabilityTargetFanMode, status).catch(device.error);
  }
}
...
async function setSwingModeFromRaw(device, raw) {
  if (raw !== Constants.NONE_VAL) {
    let status = '';
    switch (raw) {
    ...
    if (allowedValues.modes.find(element => element === status)) {
      ...
      await device.setCapabilityValue(cap, status).catch(device.error);
    }
  }
}
...
async function setPowerSelectionFromRaw(device, raw) {
  if (raw !== Constants.NONE_VAL) {
    ...
    await device.setCapabilityValue(Constants.CapabilityTargetPowerMode, status).catch(device.error);
  }
}
...
async function setMeritAFromRaw(device, raw) {
  if (raw !== Constants.NONE_VAL_HALF_BYTE) {
    ...
    await device.setCapabilityValue(Constants.CapabilityTargetMeritA, status).catch(device.error);
  }
}
...
async function setMeritBFromRaw(device, raw) {
  if (device.hasCapability(Constants.CapabilityTargetMeritB)) {
    if (raw !== Constants.NONE_VAL_HALF_BYTE) {
      ...
      await device.setCapabilityValue(Constants.CapabilityTargetMeritB, status).catch(device.error);
    }
  }
}
...
async function setAirPureIonFromRaw(device, raw) {
  if (device.hasCapability(Constants.CapabilityAirPureIon)) {
    if (raw !== Constants.NONE_VAL) {
      ...
      await device.setCapabilityValue(Constants.CapabilityAirPureIon, status).catch(device.error);
    }
  }
}
...
async function setSelfCleaningFromRaw(device, raw) {
  if (device.hasCapability(Constants.CapabilitySelfCleaning)) {
    ...
    await device.setCapabilityValue(Constants.CapabilitySelfCleaning, status).catch(device.error);
  }
}
...
async function setOutsideTemperatureFromRaw(device, value) {
  ...
  await device.setCapabilityValue(Constants.CapabilityMeasureTemperatureOutside, oTemperature).catch(device.error);
}
...
async function setInsideTemperatureFromRaw(device, value) {
  ...
  await device.setCapabilityValue(Constants.CapabilityMeasureTemperatureInside, iTemperature).catch(device.error);
  if (device.hasCapability(Constants.CapabilityMeasureTemperatureInside_8c)) {
    await device.setCapabilityValue(Constants.CapabilityMeasureTemperatureInside_8c, iTemperature);
  }
}

exports.convertStateToCapabilities = async function(device, state) {
  const bytes = convertStateToBytes(state);
  const is8C = bytes[Constants.PostionByteMeritA] === 0x04;
  ...
  setTargetTemperatureFromRaw(device, bytes[Constants.PostionByteTargetTemperature], is8C);
};
