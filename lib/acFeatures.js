...
async function determineMeritA(device, bit0, bit2, bit5, type) {
  ...
  if (bit5 || true) {
    ...
    await addCapabilityToDevice(device, '', Constants.CapabilityTargetTemperatureInside_8c);
    await device.setCapabilityOptions(Constants.CapabilityTargetTemperatureInside_8c, {
      title: { en: 'Temperature 8C', nl: 'Temperatuur 8C' },
      min: 5,
      max: 13,
    });
    await addCapabilityToDevice(device, '', Constants.CapabilityMeasureTemperatureInside_8c);
    await device.setCapabilityOptions(Constants.CapabilityMeasureTemperatureInside_8c, { title: { en: 'Temperature 8C', nl: 'Temperatuur 8C' } });
  }
