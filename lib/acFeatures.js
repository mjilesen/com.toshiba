...
async function determineEnergyConsumption(device, bit){
  if (bit){
    await addCapabilityToDevice(device, "", Constants.CapabilityEnergyConsumptionLastHour);
    await addCapabilityToDevice(device, "", Constants.CapabilityEnergyConsumptionLastDay);
  }
}
...
exports.setCapabilities = async device => {
  ...
  const meritBit15 = getBit(meritFeature, 15);
